...
resource "google_dns_record_set" "dns-a" {
  ...
  managed_zone = "${var.dns_zone_name != "none" ? data.google_dns_managed_zone.dns-zone.name : ""}"
  name         = "${var.dns_zone_name != "none" ? format("${var.owner}-${var.service}-%02d.%s",count.index+1,data.google_dns_managed_zone.dns-zone.dns_name) : ""}"
  type         = "A"
  ttl          = "${var.dns_ttl}"
