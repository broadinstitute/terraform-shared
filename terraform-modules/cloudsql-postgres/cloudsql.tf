...
#    }
    ...
    ip_configuration {
      ...
      private_network = var.private_enable == true ? data.google_compute_network.network[0].self_link : null
      require_ssl   = true
...
data "google_compute_network" network {
  count = var.private_network_self_link != "" ? 0 : 1
  provider = google.target
...
resource "google_compute_global_address" "private_ip_address" {
  ...
  prefix_length = 24
  network       = var.private_network_self_link != "" ? var.private_network_self_link : data.google_compute_network.network[0].self_link
  depends_on    = [var.dependencies]
}
